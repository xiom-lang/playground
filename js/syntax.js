...
var concepts = [
  ...
  // -- XIOM Magic --
  { id: "contracts", name: "Promises to the Compiler", icon: "\uD83E\uDD1D", section: "xiom-magic", metaphor: "Contracts are promises you write down for the compiler -- 'b must not be zero', 'the result must satisfy result * b == a'. They are runtime-checked: the guard runs when your program runs and aborts loudly if the promise is broken.", example: "fn divide(a: Float64, b: Float64) -> Float64\n  requires: b != 0.0\n  ensures: result * b == a\n{\n  return a / b;\n}\nfn main() { io.println(divide(12.0, 4.0)); }", keywords: ["requires", "ensures", "contract"], lessonId: "L4-01" },
  { id: "invariants", name: "Rules That Must Hold", icon: "\uD83D\uDEE1\uFE0F", section: "xiom-magic", metaphor: "An invariant is like a safety railing -- no matter what changes, this rule about your data must stay true, and the runtime checks it.", example: "type Health = {\n  current: Int;\n  maximum: Int;\n  invariant: current >= 0;\n  invariant: current <= maximum;\n}\nfn main() { let hp = Health{ current: 100; maximum: 100; }; io.println(hp.current); }", keywords: ["invariant", "type", "safety"], lessonId: "L4-06" },
  ...
  { id: "safety", name: "Safety by Design", icon: "\uD83D\uDEE1\uFE0F", section: "xiom-magic", metaphor: "XIOM catches many classes of bugs before your program runs -- moved values, borrow conflicts, missing cases -- and contract or overflow violations abort at runtime with a structured diagnostic instead of failing silently.", example: "fn main() {\n  // Caught at compile time:\n  // - using a variable after it moved\n  // - writing to borrowed data\n  // Caught at runtime with a structured diagnostic:\n  // - breaking a contract, integer overflow\n  io.println(\"Fail loudly, never silently.\");\n}", keywords: ["safety", "compile-time", "no-gc"], lessonId: "L4-21" },
